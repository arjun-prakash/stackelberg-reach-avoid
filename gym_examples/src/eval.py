...
def play_match(
    ...
    for episode in range(100):
        episode = episode + 1
        key = jax.random.PRNGKey(episode*salt)
        ...
        #         False,
        #         None,
        #         {'attacker':np.array([0,3,3*np.pi/2]), 'defender':np.array([0,-1,0])},
        #     ]
        # )
        ...
        #env.make_gif(f"gifs/eval/{timestamp}_{salt}_nash.gif")
