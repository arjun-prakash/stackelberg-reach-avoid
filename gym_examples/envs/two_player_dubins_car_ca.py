...
class ContinuousTwoPlayerEnv:
    ...
    def _update_state(self, state, action, player):
        ...
        new_x, new_y = self._restrict_movement(x, y, new_x, new_y, step_size)
        new_state = {**state, player: jnp.array([new_x, new_y])}
        new_nn_state = self.encode_helper(new_state)
        return new_state, new_nn_state

    def _restrict_movement(self, x, y, new_x, new_y, step_size):
        def within_radius():
            return new_x, new_y

        def outside_radius():
            theta = jnp.arctan2(new_y - y, new_x - x)
            return x + jnp.cos(theta) * step_size, y + jnp.sin(theta) * step_size

        dist = jnp.sqrt((new_x - x)**2 + (new_y - y)**2)
        return jax.lax.cond(dist <= step_size, within_radius, outside_radius)
