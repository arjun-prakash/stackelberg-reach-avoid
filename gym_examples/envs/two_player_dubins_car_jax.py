...
class TwoPlayerDubinsCarEnv(DubinsCarEnv):
    ...
    def __init__(self, game_type, num_actions, size, reward, max_steps, capture_radius, goal_position, goal_radius, timestep, v_max, omega_max):
        super().__init__()
    ...
    def _update_state(self, state, action, player):
        ...
        v_max = jax.lax.cond(player == 'attacker', lambda _: self.v_max, lambda _: self.v_max/1.75, None)
        #omega = jax.lax.cond(player == 'attacker', lambda _: self.omega_max, lambda _: self.omega_max/2, None)
